...
describe("Escrow System", () => {
  ...
  // Get program ID from IDL (set during build)
  ...
  // Escrow accounts for main flow test
  ...
  before(async () => {
    ...
    // Generate escrow seed and PDAs for main test flow
  ...
  describe("Set Arbiter", () => {
    it("Should set arbiter successfully (before deposit)", async () => {
      // Set arbiter while escrow is still in Initialized state
      const tx = await program.methods
  ...
  describe("Release Funds", () => {
    ...
    it("Should fail if seller tries to release before timeout", async () => {
      // Create new escrow
      const secureEscrowSeed = new anchor.BN(Date.now() + 250);
      const [secureEscrowPda] = anchor.web3.PublicKey.findProgramAddressSync(
        [
          Buffer.from("escrow"),
          buyerKeypair.publicKey.toBuffer(),
          secureEscrowSeed.toArrayLike(Buffer, "le", 8)
        ],
        program.programId
      );
      const [secureVaultPda] = anchor.web3.PublicKey.findProgramAddressSync(
        [
          Buffer.from("vault"),
          buyerKeypair.publicKey.toBuffer(),
          secureEscrowSeed.toArrayLike(Buffer, "le", 8)
      ...
      // Try to release as seller (should fail)
      try {
        await program.methods
          .release()
          .accounts({
        ...
        assert.include(error.message, "UnauthorizedRelease");
      }
    });

    it("Should release funds by arbiter", async () => {
      // Create new escrow for this test
      const arbiterEscrowSeed = new anchor.BN(Date.now() + 200);
      const [arbiterEscrowPda] = anchor.web3.PublicKey.findProgramAddressSync(
        [
          Buffer.from("escrow"),
          buyerKeypair.publicKey.toBuffer(),
          arbiterEscrowSeed.toArrayLike(Buffer, "le", 8)
        ],
        program.programId
      );
      const [arbiterVaultPda] = anchor.web3.PublicKey.findProgramAddressSync(
        [
          Buffer.from("vault"),
          buyerKeypair.publicKey.toBuffer(),
          arbiterEscrowSeed.toArrayLike(Buffer, "le", 8)
      ...
      await program.methods
        ...
        .accounts({
          escrow: arbiterEscrowPda,
          vault: arbiterVaultPda,
          buyer: buyerKeypair.publicKey,
          mint: mint,
        })
        .signers([buyerKeypair])
      ...
      // Set arbiter BEFORE deposit (while in Initialized state)
      await program.methods
        .setArbiter(arbiterKeypair.publicKey)
        .accounts({
          escrow: arbiterEscrowPda,
      ...
      // Verify release
      const escrowAccount = await program.account.escrowAccount.fetch(arbiterEscrowPda);
      assert.equal(escrowAccount.state.released !== undefined, true);
  ...
  describe("Cancel Escrow", () => {
    it("Should cancel escrow successfully when Initialized", async () => {
    ...
    it("Should fail to cancel escrow when Funded (Rug Pull Prevention)", async () => {
      // Create new escrow
      const rugPullSeed = new anchor.BN(Date.now() + 350);
      const [rugPullEscrowPda] = anchor.web3.PublicKey.findProgramAddressSync(
        [
          Buffer.from("escrow"),
          buyerKeypair.publicKey.toBuffer(),
          rugPullSeed.toArrayLike(Buffer, "le", 8)
        ],
        program.programId
      );
      const [rugPullVaultPda] = anchor.web3.PublicKey.findProgramAddressSync(
        [
          Buffer.from("vault"),
          buyerKeypair.publicKey.toBuffer(),
          rugPullSeed.toArrayLike(Buffer, "le", 8)
      ...
      await program.methods
        ...
        .accounts({
          escrow: rugPullEscrowPda,
          vault: rugPullVaultPda,
          buyer: buyerKeypair.publicKey,
          mint: mint,
        })
        .signers([buyerKeypair])
  ...
  describe("Close Escrow", () => {
    it("Should close escrow and recover rent", async () => {
      ...
      try {
        ...
        assert.fail("Expected escrow account to be closed");
      } catch (error) {
      ...
      assert.isNull(vaultAccountInfo, "Expected vault account to be closed");
