...
if [[ ! -f "$KEYPAIR" ]]; then
    echo -e "${RED}[ERROR] Keypair not found: $KEYPAIR${NC}"
...
if (( $(echo "$BALANCE < $MIN_BALANCE" | bc -l) )); then
    echo -e "${RED}[ERROR] Insufficient balance for $ENVIRONMENT deployment${NC}"
    ...
    if [[ "$ENVIRONMENT" == "devnet" ]]; then
        ...
        solana airdrop 5
        BALANCE=$(solana balance --output json | jq -r '.value')
        echo -e "${GREEN}[OK] New balance: $BALANCE SOL${NC}"
    else
